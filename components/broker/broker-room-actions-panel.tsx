...
export function BrokerRoomActionsPanel({ action, roomId }: BrokerRoomActionsPanelProps) {
  const formAction = updateBrokerRoomAction.bind(null, roomId);

  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-2">
        <ClipboardList className="size-4 text-teal-700" aria-hidden />
        <h3 className="text-base font-bold text-slate-950">Hành động của tôi</h3>
      </div>
      <p className="mt-1 text-sm text-slate-500">
        Dữ liệu này là riêng của bạn, không sửa thông tin phòng của landlord.
      </p>
      ...
      <form action={formAction} className="mt-4 space-y-4">
        ...
        <button
          className="inline-flex h-11 w-full items-center justify-center rounded-md bg-teal-700 px-4 text-sm font-semibold text-white hover:bg-teal-800"
          type="submit"
        >
...
function ActionCheckbox({
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">
      <input className="size-4 accent-teal-700" defaultChecked={defaultChecked} name={name} type="checkbox" />
